...
export default {
  ...
  mounted() {
    ...
    if (!this.events) {
      //get events from DB
      this.getAllEvents()
        .then(this.eventSelection)
        .then(this.eventTime)
        .then(this.checkOccurrence)
        .then(this.startTime);
    } else {
      //get current session events without those which were answered
      this.eventSelection();
      this.eventTime();
      this.startTime();
    }
  ...
  methods: {
            ...
            this.events = [
              {
                id: 1,
                description:
                  "Seu chefe o convida para um happy hour com os diretores no final do expediente.  Ao mesmo tempo, você recebe uma mensagem de seu cônjuge lembrando da apresentação no colégio do seu filho. O que você faz?",
              ...
              },
              {
                id: 2,
                description:
                  "Você tem muito trabalho a fazer, porém o tempo com sua família anda escasso. No final do expediente você escolheria jantar com sua família ou fazer hora extra?",
              ...
              },
              {
                ...
                op_work: "Leva o filho ao médico",
                op_family: "Ir para a reunião",
                min_hour: 9,
                max_hour: 18
              },
              {
              ...
              },
              {
                id: 5,
                description:
                  "Final de ano, seu chefe chama você para negociar suas férias. Porém, ele apresenta uma proposta de trabalho extra. Você sabe que seria bom para seu futuro na organização. O que você faz?",
              ...
              },
              {
                id: 6,
                description:
                  "Você está coordenando um novo projeto, porém seu cônjuge o convida para uma viagem de final de semana. Neste final de semana, você escolhe avançar o projeto ou viajar com família?",
              ...
              },
              {
                id: 7,
                description:
                  "Você está jantando com sua família, quando de repente recebe uma ligação de seu chefe. Você atende imediatamente ou retorna depois do jantar?",
                op_work: "Retorna depois",
                op_family: "Atende",
                min_hour: 20,
                max_hour: 23
              },
              {
                id: 8,
                description:
                  "Seu chefe lhe oferece uma proposta de transferência sua e de sua família para outra cidade com um significativo aumento de salário.  Quando discutido com a família seu companheiro o lembra que seu filho finalmente se adaptou à escola. O que você escolhe?",
              ...
              },
              {
                id: 9,
                description:
                  "Na próxima segunda-feira é seu aniversário de casamento, porém você tem agendada uma viagem de negócios. O que você faz?",
                op_work: "Viagem de negócio",
              ...
              },
              {
                id: 10,
                description:
                  "Os pais de seu cônjuge estão na cidade por uma noite, mas você tem que terminar um relatório com urgência. Você sai para jantar com a família do seu cônjuge ou faz hora extra?",
              ...
              },
              {
                id: 11,
                description:
                  "Depois de uma reorganização dos horários da empresa, você poderá trocar de turno. Caso queira este novo horário, não poderá buscar seus filhos na escola. Você aceita?",
              ...
              },
              {
                id: 12,
                description:
                  "Você está preparando uma apresentação de um projeto, porém seu filho menor pede para brincar com você. O que faz?",
    ...
    eventTime() {
      ...
      while (true) {
        ...
        if (
          selectedHour != this.firstEvent.hour ||
          this.days[selectedDay] != this.firstEvent.day
        ) {
      ...
      while (true) {
        ...
        if (
          (selectedHour != this.firstEvent.hour ||
            this.days[selectedDay] != this.firstEvent.day) &&
          (selectedHour != this.secondEvent.hour ||
            this.days[selectedDay] != this.secondEvent.day)
        ) {
    ...
    randomizeInterval(event) {
      //gets two new parameters from an event (min_hour and max_hour) and returns a value between them
    ...
    checkOccurrence() {
      if (
        this.currentHour == this.firstEvent.hour &&
        this.currentDay == this.firstEvent.day
      ) {
      ...
      if (
        this.currentHour == this.secondEvent.hour &&
        this.currentDay == this.secondEvent.day
      ) {
        this.activeEvent = true;
      ...
      if (
        this.currentHour == this.thirdEvent.hour &&
        this.currentDay == this.thirdEvent.day
      ) {
